...
BAK_DIR = Path(__file__).resolve().parent.parent / "python_scripts_bak"
...
def build_specs(decode_tiers: list[str]) -> list[dict]:
    ...
    for tier in decode_tiers:
        ...
        specs.append({
            "name": f"decode[{tier}]", "target": "codeblock_decode_cost_sum", "group_col": "CodeBlocks",
            "lin_features": feats, "tree_features": feats,
        })
    specs.append({
        "name": "frontend", "target": "pusch_detection_frontend_task_work_sum_cost",
